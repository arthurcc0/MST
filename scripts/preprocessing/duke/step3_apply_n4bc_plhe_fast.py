...
def process_patient_directory(patient_dir, input_root, output_root, mask_root, apply_n4bc=True, apply_plhe=True):
    ...
    try:
        ...
        pre_output.save(output_dir / f"pre{suffix_str}.nii.gz")
        ...
        post_output.save(output_dir / f"post_1{suffix_str}.nii.gz")
        ...
        sub_output.save(output_dir / f"sub{suffix_str}_masked.nii.gz")
...
if __name__ == "__main__":
    # Input directory containing cropped pre and post_1 images (from step2b_crop_or_pad.py)
    path_root_in = Path(r'\\rad-maid-004\D\Duke-Cancer_MRI\preprocessed_crop-with-a\data')
    
    # Output directory for processed images
    path_root_out = Path(r'\\rad-maid-004\D\Duke-Cancer_MRI\preprocessed_crop_n4bc_plhe_fast-with-a')
    ...
    if path_root_in.exists():
        ...
        print(f"TESTING MODE: Processing only first {len(patient_dirs)} directories")
        
    else:
        print(f"Input directory does not exist: {path_root_in}")
        print("Please update the path_root_in variable to point to your cropped images")
        sys.exit(1)
    ...
    # Option 1: Multi-CPU processing (reduced to 2 processes for stability)
    ...
    print(f"Starting FAST processing with N4BC={'ON' if APPLY_N4BC else 'OFF'}, PLHE={'ON' if APPLY_PLHE else 'OFF'}")
    print(f"Input: {path_root_in}")
    print(f"Output: {path_root_out_data}")
    print(f"Masks: {path_mask_root}")
    
    with Pool(processes=2) as pool:  # Reduced from 4 to 2 processes
        for _ in tqdm(pool.imap_unordered(partial_process, patient_dirs), total=len(patient_dirs)):
            pass

    # Option 2: Single-CPU processing (uncomment if needed)
    # for patient_dir in tqdm(patient_dirs):
    #     process_patient_directory(
    #         patient_dir, 
    #         str(path_root_in), 
    #         str(path_root_out_data), 
    #         str(path_mask_root),
    #         APPLY_N4BC, 
    #         APPLY_PLHE
    #     )
        
    print("FAST N4BC and PLHE processing completed!")
