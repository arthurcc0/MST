...
def _embedding_vec_from_npz(npz_path: Path) -> np.ndarray:
    """Load one fused embedding vector from main_predict --save_embeddings output."""
...
def _resolve_predict_paths(
    ...
    """Resolve embeddings_fused/ and labels CSV from a main_predict output folder."""
...
class KANClassifier(nn.Module):
    """Thin wrapper around pykan's KAN that returns [B] logits for BCEWithLogitsLoss."""
    def __init__(self, input_dim, hidden_dim=16, grid=3, k=3, seed=42, device='cpu'):
        super().__init__()
        self.kan = KAN(width=[input_dim, hidden_dim, 1], grid=grid, k=k, seed=seed, device=device)
...
def _train_kan_once(
    ...
    results_dir: Path,
    tag: str = '',
) -> tuple[KANClassifier, StandardScaler, list[float], list[float]]:
    """Train one KAN with early stopping on val; return model, scaler, loss curves."""
    ...
    model = KANClassifier(
        input_dim=input_dim, hidden_dim=16, grid=3, k=3, seed=SEED, device=str(device)
    ).to(device)
...
def _eval_kan(
    ...
    return y, probs.cpu().numpy(), preds.cpu().numpy()
...
def run_penn_split_eval(
    ...
    """Train KAN on train embeddings, early-stop on val, evaluate on held-out test."""
    ...
    print(
        f"Penn-split KAN (MST fold {fold}): train n={len(y_train)} (pos={int(y_train.sum())}), "
        f"val n={len(y_val)} (pos={int(y_val.sum())}), test n={len(y_test)} (pos={int(y_test.sum())})"
    )

    model, scaler, _, _ = _train_kan_once(
    ...
    _, test_probs, test_preds = _eval_kan(
    ...
    ax.set_title(f"KAN on MST embeddings — penn_split fold {fold}")
    ax.legend(loc='lower right')
    fig.tight_layout()
    fig.savefig(results_dir / f'roc_kan_test_fold{fold}_{SEED}.png', dpi=400)
    plt.close(fig)

    out = test_df[['uid', 'UID', 'Split', 'GT']].copy()
    out['KAN_pred'] = test_probs
    out['KAN'] = test_preds
...
def run_internal_cv(
    ...
    for i, (train_idx, val_idx) in enumerate(cv.split(X, y, groups=labels_df['base_id'])):
        ...
        model, scaler, train_losses, val_losses = _train_kan_once(
        ...
        batch_labels, batch_probs, batch_preds = _eval_kan(
            model,
            scaler,
            X.iloc[val_idx].values,
            y.iloc[val_idx].values,
    ...
    ax.set(xlabel='False Positive Rate', ylabel='True Positive Rate', title='Cross-validated ROC - KAN')
    ax.legend(loc='lower right')
    fig.savefig(results_dir / f'roc_kan_{SEED}.png', dpi=400)
    plt.close(fig)
    ...
    plt.ylabel('True')
    plt.title('Pooled Confusion Matrix - KAN')
    plt.savefig(results_dir / f'norm_cm_kan_{SEED}.png')
    plt.close()


def main() -> None:
    parser = argparse.ArgumentParser(
        description='Train KAN on fused embeddings from main_predict.py (--save_embeddings).'
    )
    ...
    run_suffix = f'bs{BATCH_SIZE}_lr{LR}_es{PATIENCE}'
    if USE_SMOTE:
        run_suffix += '_SMOTE'
    run_suffix += f'_{cli.eval_mode}_fold{cli.fold}_{SEED}'
    results_dir = Path(f'./results_kan/{mst_model_name}/{run_suffix}')
