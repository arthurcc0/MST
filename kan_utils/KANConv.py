...
class KAN_Convolutional_Layer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if len(self.kernel_size) == 1:
            return convolution.multiple_convs_kan_conv1d(x, self.convs, self.kernel_size[0], self.out_channels, self.stride, self.dilation, self.padding, self.device)
        else:
