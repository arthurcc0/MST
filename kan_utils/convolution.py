...
def calc_out_dims_1d(matrix, kernel_side, stride, dilation, padding):
    batch_size, n_channels, l_in = matrix.shape
    l_out = np.floor((l_in + 2 * padding[0] - dilation[0] * (kernel_side - 1) - 1) / stride[0]).astype(int) + 1
    return l_out, batch_size, n_channels

def multiple_convs_kan_conv1d(matrix, 
             kernels, 
             kernel_side,
             out_channels,
             stride= (1,),
    ...
    l_out, batch_size, n_channels = calc_out_dims_1d(matrix, kernel_side, stride, dilation, padding)
    matrix_out = torch.zeros((batch_size, out_channels, l_out)).to(device)
    unfold = torch.nn.Unfold(kernel_size=(1, kernel_side), dilation=dilation, padding=padding, stride=stride)

    # Reshape matrix to be 4D for unfold, then reshape back
    reshaped_matrix = matrix.unsqueeze(2) # Add a dummy height dimension
    conv_groups = unfold(reshaped_matrix).view(batch_size, n_channels, kernel_side, l_out).transpose(2, 3)

    kern_per_out = len(kernels) // out_channels
    for c_out in range(out_channels):
        out_channel_accum = torch.zeros((batch_size, l_out), device=device)
        for k_idx in range(kern_per_out):
            kernel = kernels[c_out * kern_per_out + k_idx]
            conv_result = kernel.conv.forward(conv_groups[:, k_idx, :, :].flatten(0, 1))
            out_channel_accum += conv_result.view(batch_size, l_out)
